...
parser.add_argument('--initorth', action='store_true', help='fixorh initiation')
parser.add_argument('--dataset', default='CIFAR10', type=str, help='CIFAR10 CIFAR100 MNIST')
parser.add_argument('--adam', action='store_true', help='train with Adam')
...
#         _, predicted = torch.max(outputs.data, 1)
#         total += targets.size(0)
#         correct += (lam * predicted.eq(targets_a.data).float()).cpu().sum() + (
#                     (1 - lam) * predicted.eq(targets_b.data).float()).cpu().sum()
#         acc = 100. * float(correct) / float(total)
...
#         with open(logname, 'a') as logfile:
#             logwriter = csv.writer(logfile, delimiter=',')
#             logwriter.writerow([epoch, lr, train_loss/(batch_idx+1), acc, test_loss, test_acc])